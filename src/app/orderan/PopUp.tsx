...
import {
  Button, Card, CardBody, CardHeader, Dialog, DialogBody, DialogFooter,
...
export function PopUp( { clickPopUp, setClickPopUp, onCreate, data, notify }: {
  clickPopUp: boolean,
      ...
      <div className="z-0">
        ...
        >
          ...
          <div className="relative w-full ">
            ...
            <DialogBody
              className="relative rounded-lg shadow-md shadow-slate-500  bg-slate-50 ">
              ...
              <div className="px-10 py-5 space-y-6">
                <div className="  flex gap-5 flex-col sm:flex-row">
                  <Card variant="gradient" color="blue"
                    ...
                    <Typography color="black">Nama
                      Penerima: { data.penerima }</Typography>
                    <Typography color="black">Hp
                      Penerima: { data.hpPenerima }</Typography>
                    <Typography color="black">Alamat Penerima
                      : { data.alamatPenerima }</Typography>
                    <Typography color="black">Tanggal Pesan
                      : { data.pesan.toString() }</Typography>
                    <Typography color="black">Tanggal Kirim
                      : { data.kirim.toString() }</Typography>
                    <Typography color="black">Tanggal waktu Kirim
                      : { data.waktuKirim.toString() }</Typography>
                    <Typography color="black">Keterangan Lokasi
                      : { data.lokasi }</Typography>
                  </Card>
                  ...
                  <Card
                    ...
                    <Typography color="black">Lokasi
                      : { data.lokasi }</Typography>
                    <Typography color="black">Travel Pengirim
                      : { data.pengirim }</Typography>
                    <Typography color="black">Ekspedisi
                      : { data.ekspedisi }</Typography>
                    <hr/>
                    <Typography color="black">Ongkir
                      : { Rupiah( data.ongkir ) }</Typography>
                    ...
                    <Typography color="black">Semua Harga Item
                      : { Rupiah( data.hitung.semuaHargaItem ) }</Typography>
                    <Typography color="black">Semua Harga Produk
                      : { data.hitung.semuaHargaProduct &&
                        Rupiah( data.hitung.semuaHargaProduct ) }</Typography>
                    <Typography color="black">Semua Harga
                      Total: { Rupiah( data.hitung.totalHarga ) }</Typography>
                          ...
                          <div
                            ...
                            <CardBody className=" rounded p-2 ">
                              <Typography variant="h6" color="black"
                                          className="mb-2">{ item.nama }</Typography>
                              <Typography
                                color="black">{ Rupiah( item.harga ) }</Typography>
                              <Typography color="black">Jenis
                                : { item.jenis }</Typography>
                              <Typography color="black"> Jumlah
                                : { item.jumlah }</Typography>
                            </CardBody>
                          </div>
                        </Card>
                      )
              ...
              <DialogFooter
                ...
                {/*<button*/ }
                {/*  onClick={ () => notify( getData.msg, 'success' ) }>*/ }
                {/*  click*/ }
                {/*</button>*/ }
